...
const Write = ()=>{
  ...
  const handleSubmit = async (e)=> {
    ...
    const data = {
      ...
      content: value,  // ReactQuill content
    };
    ...
    <div className='h-[calc(100vh-64px)] md:h-[calc(100vh-80px)] flex flex-col gap-6'>
      ...
      <Upload type="image" setProgress={setProgress} setData={setCover} >
        ...
        <div className="flex items-center gap-4">
          <label htmlFor="" className='text-sm'>Chose a category</label>
        ...
        <button disabled={mutation.isPending || (0 < progress && progress < 100)} className='bg-blue-800 text-white font-medium rounded-xl mt-4 p-2 w-36 disabled:bg-blue-400 disabled:cursor-not-allowed'> {mutation.isPending ? "Loading...": "Send"} </button>
        {"Progress:" + progress}
